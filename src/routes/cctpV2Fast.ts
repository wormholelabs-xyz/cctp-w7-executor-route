...
import { CCTPv2ExecutorRoute, CCTPv2QuoteDetails } from "./cctpV2Standard";
  ...
  implements routes.StaticRouteMethods<typeof CCTPv2FastExecutorRoute>
{
  ...
  async quote(
    ...
    const chains = CCTPv2FastExecutorRoute.supportedChains(fromChain.network);
    if (!chains.includes(fromChain.chain) || !chains.includes(toChain.chain)) {
      return {
        success: false,
        ...
        try {
          ...
          if (isFailed(receipt)) {
            if (!receipt.attestation) {
              ...
              // on the receipt before checking for failed status
